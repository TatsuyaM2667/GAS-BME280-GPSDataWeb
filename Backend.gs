// Google Apps Script コード（修正版＋データ初期化機能追加）
...
function getLatestData() {
 try {
   ...
   // 最後の行から逆順に検索し、有効なデータを持つ行を見つける
   let currentData = null;
   for (let i = values.length - 1; i >= 1; i--) {  // インデックス1から開始（ヘッダーをスキップ）
     const row = values[i];
    
     // 温度、湿度、気圧のいずれかが有効な数値（0以外）であるかを確認
...
// グラフ用データを取得する関数（HTMLファイルで呼び出される）- 全データ取得版
